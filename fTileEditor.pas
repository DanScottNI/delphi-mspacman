...
begin
  if pMode = LEVEL then
  begin
    DaTile := ROMData.Export8x8Pat(TileID);
  end;
...
begin
  ...
  try
    ...
    if pMode = LEVEL then
    begin
      TempBitmap.FillRect(0,0,25,25, ROMData.NESPal[ROMData.Palette[EditorConfig.LastPaletteTileEditor,0]]);
      TempBitmap.FillRect(25,0,50,25, ROMData.NESPal[ROMData.Palette[EditorConfig.LastPaletteTileEditor,1]]);
      TempBitmap.FillRect(50,0,75,25, ROMData.NESPal[ROMData.Palette[EditorConfig.LastPaletteTileEditor,2]]);
      TempBitmap.FillRect(75,0,100,25, ROMData.NESPal[ROMData.Palette[EditorConfig.LastPaletteTileEditor,3]]);
    end;
...
begin
  ...
  try
    ...
    begin
      for x := 0 to 7 do
        for i := 0 to 7 do
          TempBitmap.Pixel[x,i] := ROMData.NESPal[ROMData.Palette[EditorConfig.LastPaletteTileEditor,DaTile.Pixels[i,x]]];
...
begin
  ...
  begin
    if EditorConfig.LastPaletteTileEditor = 3 then
      EditorConfig.LastPaletteTileEditor := 0
    else
      EditorConfig.LastPaletteTileEditor := EditorConfig.LastPaletteTileEditor +1;
...
begin
  if pMode = LEVEL then
    ROMData.Import8x8Pat(TileID,DaTile);
{  else if pMode = TITLESCREEN then
