...
type
  ...
  private
    ...
    procedure SetMirrorLevels(const Value: Boolean);
  ...
  public
    ...
    property MirrorLevels : Boolean read _MirrorLevels write SetMirrorLevels;
  end;
...
begin
  ...
  try
    ...
    INI.WriteBool('General','MirrorLevels',_MirrorLevels);
    INI.UpdateFile;
  finally
    FreeAndNil(INI);
  end;
...
//    _LeftTextColour := INI.ReadInteger('General','LeftTextColour',$FFFF0000);
    ...
    _DataFile := INI.ReadString('General','DataFile','Ms. Pac-Man (U) (Namco).ini');
    ...
    _MirrorLevels := INI.ReadBool('General','MirrorLevels', False);     
    SetupMRU();
...
begin
  result := ExtractFileDir(Application.ExeName) + '\Resources\Palettes\' + _Palette;
end;
...
begin
  result := ExtractFileDir(Application.ExeName) + '\Resources\' + _DataFile;
end;
...
procedure TEditorConfig.SetMirrorLevels(const Value: Boolean);
begin
  _MirrorLevels := Value;
  _Changed := True;
end;
